...
class HTMLReportGenerator:
    ...
    <body>
        <div class="container">
            ...
            <div class="footer">
                ...
                <p>💾 Save this page to keep the report</p>
            </div>
        </div>
    ...
    def generate_report(
        ...
        if difficulty_level not in _VALID_CEFR:
            difficulty_level = 'B1'
            ...
            for level, words in vocab_by_level.items():
                if words and len(words) > 0:
                    ...
                    for word in list(words)[:10]:
                        vocab_html += f'<span class="phrase-tag {css_class}">{html_escape.escape(str(word))}</span>'
                    vocab_html += '</div>'
